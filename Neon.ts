const Neon:HTMLCanvasElement = document.querySelector("#NeonClock");
const EmptyButton:HTMLSpanElement = document.querySelector("#EmptyButton");
const neonCtx:CanvasRenderingContext2D = Neon.getContext("2d",{willReadFrequently: true});
Neon.width = 916;
Neon.height = 200;
...
class Drawer{
	...
	segmentElement:number;
	NeonCtx:CanvasRenderingContext2D;
	...
	DrawVerticalDigit(x:number,y:number){
		this.PrepareToDraw(x,y);
		const Points:{x:number,y:number}[] = [
			{x: x+this.Inch, 	y:y+this.Inch},
	...
	DrawColonSequence(x:number,y:number){
		this.PrepareToDraw(x,y);
		x +=this.Inch * 5.3;
...
class Seg7NeonDisplay{
	...
	AdjustDefaultPosition(Vector2:{x:number,y:number}){
	...
	CreateNumber(number){
		...
		const SegMap:number = segments[number];
...
class GlitchEffecter{
	BlueNeon:HTMLCanvasElement;
	RedNeon:HTMLCanvasElement;
	...
	BuildClone(CloneNeonCtx:CanvasRenderingContext2D
		,fillcolor:string,strokecolor:string,Text:string){
		...
		*/
	BuildQueSequence(minBoxes:number,MaxBoxes:number){
	...
	Glitch(sequence:number,Ctx:CanvasRenderingContext2D){
		const array:number[] = this.RenderBox[this.AnimationLength-sequence];
		if(array === undefined){
			return;
		}
	...
	RemoveClone(CloneNeonCtx:CanvasRenderingContext2D){
		CloneNeonCtx.clearRect(0,0,916,200);
	}
}
const startTime = Date.now();
function getAppUsageTime(): string {
