...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const AppButton = Array.from(document.querySelectorAll(".AppButton"));
    const LinkButton = Array.from(document.querySelectorAll(".LinkButton"));
    if (AppButton) {
...
function AsignText(Fri, Thu, Wed, Tue, Mon) {
    ...
    const DailysSchedule = Contexts[Today - 1];
...
class MySchedule {
    ...
    getIntTime(number) {
        const Target = this.Daily[number];
        const HH = Target.time.slice(0, 2);
        const MM = Target.time.slice(3);
        const IntTime = { Hour: parseInt(HH), Minute: parseInt(MM) };
        return IntTime;
    }
}
RequireTextdata();
